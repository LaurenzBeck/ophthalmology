...
class Involution(nn.Module):
    """
    Implementation of `Involution: Inverting the Inherence of Convolution for Visual Recognition` from:
    https://github.com/shuuchen/involution.pytorch/blob/main/involution.py
    """
    ...
    def __init__(
        ...
        self.span = nn.Conv2d(
            channels_reduced, kernel_size * kernel_size * groups, 1
        )
    ...
    def forward(self, input_tensor):
        ...
        if self.stride > 1:
            out_size = (
                lambda x: (x + 2 * self.padding - self.kernel_size)
                // self.stride
                + 1
            )
            height, width = out_size(height), out_size(width)
        uf_x = rearrange(
            self.unfold(input_tensor),
            "b (g d k j) (h w) -> b g d (k j) h w",
            g=self.groups,
            k=self.kernel_size,
            j=self.kernel_size,
            h=height,
        ...
        kernel = rearrange(
            self.span(self.reduce(input_tensor)),
            "b (k j g) h w -> b g (k j) h w",
            k=self.kernel_size,
            j=self.kernel_size,
        )

        out = rearrange(
            torch.einsum("bgdxhw, bgxhw -> bgdhw", uf_x, kernel),
            "b g d h w -> b (g d) h w",
        )
...
class Involution2d(nn.Module):
    ...
    code from:
    https://github.com/ChristophReich1996/Involution/blob/master/involution/involution.py
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        ), "in channels must be a positive integer."
        assert (
            in_channels % groups == 0
        ), "out_channels must be divisible by groups"
        assert (
            isinstance(out_channels, int) and out_channels > 0
        ), "out channels must be a positive integer."
        assert (
            out_channels % groups == 0
        ), "out_channels must be divisible by groups"
        assert (
            isinstance(sigma_mapping, nn.Module) or sigma_mapping is None
        ...
        assert isinstance(stride, int) or isinstance(
            stride, tuple
        ), "stride must be an int or a tuple of ints."
        assert isinstance(groups, int), "groups must be a positive integer."
        assert (
            isinstance(reduce_ratio, int) and reduce_ratio > 0
        ), "reduce ratio must be a positive integer."
        assert isinstance(dilation, int) or isinstance(
            dilation, tuple
        ...
        self.sigma_mapping = (
            ...
            if sigma_mapping is not None
                ...
                nn.BatchNorm2d(
                    num_features=self.out_channels // self.reduce_ratio,
                    momentum=0.3,
                ),
                nn.ReLU(),
            )
        ...
        self.o_mapping = nn.AvgPool2d(
            kernel_size=self.stride, stride=self.stride
        )
    ...
    def __repr__(self) -> str:
        ...
        """
        return "{}({}, {}, kernel_size=({}, {}), stride=({}, {}), padding=({}, {}), " "groups={}, reduce_ratio={}, dilation=({}, {}), bias={}, sigma_mapping={})".format(
