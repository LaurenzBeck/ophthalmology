...
class SimCLRAug(torch.nn.Module):
    def __init__(
        ...
        resize_ratio=tuple[float, float](0.75, 1.3333333333333333),
        rotate_deg: int = 30,
        jitter_s: float = 0.6,
        blur_s: tuple[int, int] = (4, 32),
        ...
        bw_p: float = 0.3,
        blur_p: float = 0.3,
        stats=tuple[list[float, float, float], list[float, float, float]](
        ...
        self.aug = simclr.get_simclr_aug_pipelines(
            size,
            rotate=rotate,
            jitter=jitter,
            bw=bw,
